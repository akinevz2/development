...
globbit:
	...
	@if git diff --cached --quiet; then \
		...
		git commit; \
		if grep -i "master" - <<<$(git branch -v) ; then ; echo "branch main doesn't exist for $(pwd)" ; return 1; \
...
# Default target
.PHONY: all install-scala clean install
...
# Load devcontainer.json, extract postStartCommand, assert it is a string, and execute it.
# Usage: make poststart
reinstall:
